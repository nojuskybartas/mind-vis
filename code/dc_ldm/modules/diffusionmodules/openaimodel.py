...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        activation_function_conditioning=None,
    ):
        super().__init__()
        ...
        if activation_function == "relu":
            ...
            raise ValueError(
                f"activation_function {activation_function} not implemented. Please choose 'relu' | 'silu'"
            )

        if activation_function_conditioning == "relu":
            self.activation_function_conditioning = nn.ReLU()
        elif activation_function_conditioning == "silu":
            self.activation_function_conditioning = nn.SiLU()
        elif activation_function_conditioning is None:
            self.activation_function_conditioning = nn.Identity()
        else:
            raise ValueError(
                f"activation_function_conditioning {activation_function_conditioning} not implemented. Please choose 'relu' | 'silu' | None"
            )

        self.time_embed = nn.Sequential(
            linear(model_channels, time_embed_dim),
            self.activation_function_conditioning,
