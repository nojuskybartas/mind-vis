...
class fLDM:
    ...
    def __init__(
        ...
        config.model.params.unet_config.params.activation_function = activation_function
        config.model.params.unet_config.params.activation_function_conditioning = (
            activation_function_conditioning
        )
        config.model.params.loss_type = loss_function
        ...
        model.cond_stage_model = cond_stage_model(
            metafile, num_voxels, self.cond_dim, global_pool=global_pool
        )
    ...
    def finetune(
        ...
        test_loader = DataLoader(
            test_dataset, batch_size=len(test_dataset), shuffle=False, num_workers=64
        )
    ...
    @torch.no_grad()
    def generate(
        self, fmri_embedding, num_samples, ddim_steps, HW=None, limit=None, state=None
    ):
