...
class fLDM:
    ...
    def __init__(self, metafile, num_voxels, device=torch.device('cpu'),
        ...
        self.fmri_model = cond_stage_model(metafile, num_voxels, self.cond_dim, global_pool=global_pool)
        self.ldm_pipeline = DiffusionPipeline.from_pretrained("facebook/DiT-XL-2-256")


        self.fmri_model.cond_stage_trainable = True
        self.fmri_model.cond_stage_model = cond_stage_model(metafile, num_voxels, self.cond_dim, global_pool=global_pool)
    ...
    def finetune(self, trainers: pl.Trainer, dataset, test_dataset, bs1, lr1,
        ...
        print('\n##### Stage One: only optimize conditional encoders #####')
        dataloader = DataLoader(dataset, batch_size=bs1, shuffle=True, num_workers=64)
        test_loader = DataLoader(test_dataset, batch_size=len(test_dataset), shuffle=False, num_workers=64)
